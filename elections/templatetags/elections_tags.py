...
def do_get_hot_races(parser, token):
    ...
    try:
        ...
        raise template.TemplateSyntaxError("%s tag shoud be in the form: %s" % (tag_name, proper_form))
...
class HotRacesNode(template.Node):
    ...
    def render(self, context):
        ...
        if self.featured:
            ...
            hot_race_qs = hot_race_qs.filter(featured=to_bool(resolve(self.featured, context)))
        context[self.var_name] = hot_race_qs
        
        return ''

def get_latest_objects_by_content_type(hot_race, app_label, model_name, relation_type_include_list,
