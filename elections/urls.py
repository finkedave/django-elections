...
urlpatterns += patterns('',
    ...
    url(r'^(?P<state>[A-Z][A-Z])/live-map/(?P<year>\d{4})/(?P<slug>[a-zA-Z0-9_-]+)/$', 'elections.views.live_map'),
    ...
    url(r'^delegate-tracker/(?P<year>\d{4})/(?P<slug>[a-zA-Z0-9_-]+)/$', 'elections.views.delegate_tracker'),
    url(r'^delegate-tracker/(?P<year>\d{4})/$', 'elections.views.delegate_tracker'),
